...
> GitHub Pagesとの違い：Cloudflare Pagesは**Privateリポジトリでも無料で公開**できる。試行錯誤の履歴を人に見られたくない場合に向いている。
...
- GitHubアカウント作成済み
- リポジトリ `my-first-site` 作成済み（Public / Private どちらでもOK）
...
1. [cloudflare.com](https://www.cloudflare.com/) にアクセスして `Sign up` をクリック
2. メールアドレスとパスワードを入力して登録（Googleアカウント連携も可）
3. メール認証を済ませてログイン

### 3-2. Cloudflare PagesとGitHubを連携する

1. ログイン後、左メニューの `Workers & Pages` を選択

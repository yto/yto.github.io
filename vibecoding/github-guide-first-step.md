...
アカウント登録からファイル保存まで、すべてブラウザだけで行う。
...
| Public / Private | Public（公開） or Private（非公開）を選ぶ |
| Add a README file | 今回はチェックしない |
...
5. `https://ユーザー名.github.io/my-first-site/` というURLが発行される
...
| `git commit` | 「この状態をセーブする」。セーブデータに名前をつけるイメージ。 |
| `git push` | セーブデータをGitHubにアップロード。 |
...
# 初回はブラウザが開いてGitHubのログイン・認証を求められる場合がある

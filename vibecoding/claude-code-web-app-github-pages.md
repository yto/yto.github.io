...
- **自分一人が使う（ローカル完結型）**：データをブラウザ内に保存。サーバー不要。電卓、タイマー、TODO、メモ帳、パズルゲーム、カスタマイズ設定の保存など。
- **みんなで使う（サーバー連携型）**：データをサーバーに保存。ネット接続が必須。チャット、SNS、同時参加型ゲーム、スコアランキングなど。

今回のハンズオンでは、**自分一人が使うアプリ**を作ります。

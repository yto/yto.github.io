...
- ローカル保存型（ブラウザ完結）
  - データはブラウザ内に保存されます。基本的にその端末だけで使う、自分一人が使う前提です。
  - 例：TODO、メモ帳、個人用ツール、オフライン対応アプリ
- サーバー保存型（クラウド連携）
  - データはサーバーに保存されます。基本的にみんなで使う（複数ユーザー・複数端末）前提です。
  - 例：SNS、チャット、共同編集、ランキング

なお、設定保存のような機能はどちらにも実装できます。
あくまで違いは「データの本体がどこにあるか」です。

そして、この違いによって、アプリの作りやすさも大きく変わります。
ローカル保存型はシンプルに作れますが、サーバー保存型はデータ管理や通信処理が必要になり、構成が複雑になります。

今回のハンズオンでは、ローカル保存型（自分一人が使うアプリ）を作ります。

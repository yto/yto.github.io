...
GitHubにpushすると、**GitHub Actions**がD1のマイグレーションからPages・Pages Functionsのデプロイ（本番環境への公開・反映）まで一括で実行します。ローカルでのDB作成やログインには、Cloudflare公式CLIの**Wrangler**を使います。
...
Node.js と Wrangler はローカルでの開発・D1操作、そしてGitHub Actionsでの自動デプロイ、すべての基盤となるツールである。

### Node.js のインストール

Wrangler は Node.js 上で動くため、先にインストールしておく。

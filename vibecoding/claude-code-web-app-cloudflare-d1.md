...
1つ目は、**Claude Codeが代わりにできない手作業があるから**です。CloudflareダッシュボードでのD1バインディングの設定、GitHub SecretsへのAPI Token登録など、ブラウザ上でのクリック操作はClaude Codeには頼めません。一発依頼でコードが完成しても、この設定を知らないと動かせません。

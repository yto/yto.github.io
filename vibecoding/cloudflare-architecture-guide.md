...
GitHubへpushするだけで、D1マイグレーション（DB構造の更新）からPages・Pages Functionsのデプロイ（本番環境への公開）までが自動で実行されます。
...
- D1マイグレーション（DB構造の更新） → Pages/Pages Functionsのデプロイの順で実行される
...
Wranglerはプロジェクトの最初から使います。セットアップ時にはCloudflareへのログインと、D1データベースの作成に使います。開発中は、Pages Functionsの動作をローカルで確認したり、D1のデータを直接操作したり、UIとAPIを合わせて手元で動作確認する際にも活用します。

日常のデプロイはGitHub Actionsが担います。mainブランチへpushすると自動でデプロイが実行されます。この処理の中でもwranglerが動いていますが、自動実行されるため手動操作は不要です。ローカルPCからwranglerで直接デプロイすることはありません。

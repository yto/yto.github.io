...
[前回のハンズオン](cloudflare-pages-static-files.html)は、Cloudflareのダッシュボードから静的ファイルをドラッグ＆ドロップして公開した。今回はその発展として、コマンド操作（CLI）でデプロイできるようになることを目指す。

## 1. この回でやること

- Claude CodeでWebページを作る
- 公開用フォルダ（`public`）を使う
- `wrangler` コマンドでCloudflare Pagesにデプロイする

## 2. Node.js と Wrangler のセットアップ

この章ではローカルマシンに Wrangler をインストールする。Wrangler は初期セットアップや、開発中のローカルプレビューに使う。なお、Wrangler は Node.js 上で動くため、Node.js も合わせてインストールする。
...
## 4. Claude Codeで開発を始める

Claude Desktopを起動し、この作業フォルダを指定する。
...
デプロイメントURLと本番URLをブラウザで開き、変更が反映されていることを確認する。
